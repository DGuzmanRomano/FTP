...
window.addEventListener("DOMContentLoaded", async () => {
  ...
  // --- UI Update Function ---
  const setFileReady = (filePath: string) => {
  ...
  dropZone.addEventListener('click', async () => {
    ...
    if (selectedFilePath || dropZone.classList.contains('is-processing')) return;
    try {
  ...
  listen<string[]>('tauri://file-drop', (event: Event<string[]>) => {
